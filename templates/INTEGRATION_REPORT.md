...
- 失败测试列表：{{...}}（如有）
- 跳过的外部依赖类测试集（仅在本地无法启动 docker / 真实下游时填）：{{包/标签 + 跳过原因}}
  > 默认 build tag 下普通单测必须 100% 通过；带 `//go:build integration` 等标签的测试若需真实下游，可登记跳过，但**不视为已验证**。
...
> **范围说明**：本表只对**本地可静态/构建/测试验证**的需求出"通过/不通过"结论；对必须线上才能验证的需求（灰度真实生效、报警真实触发、降级在真实拓扑下回切、K8s rollback 演练等），仅登记"代码/配置层面是否已落地"，不出"通过"结论，端到端验证交由独立的发布/SRE 流程处理。
...
| 日志 | 结构化 + trace_id | ✅ | 全部 | — |
| 报警 | 错误率 > 1% 报警 | ⚪ 仅静态登记 | T-XXX | 配置已写入；端到端触发不在本阶段验证范围 |
| 灰度 | 按 user_id 灰度 | ⚪ 仅静态登记 | T-XXX | 路由代码已落地；真实生效需线上验证 |
| 回滚 | 支持快速回滚 | ⚪ 仅静态登记 | — | 依赖部署平台能力，本阶段不演练 |
| 降级 | 优惠券服务降级 | ⚪ 仅静态登记 | T-XXX | 降级开关代码已落地；真实切换需线上验证 |
| 幂等 | 创建订单幂等 | ✅ | T-009 | — |
...
> **范围说明**：本结论仅覆盖**本地可静态/构建/测试验证**的范围。是否真正上线，还需经独立的发布/SRE 流程（线上 dry-run、灰度、报警、回滚演练等），**不在本 skill 出结论范围内**。

☐ ✅ **PASS**：§1~§5、§8 全部通过；§6 一致性问题清单仅含 Minor；本地校验门禁放行
☐ ⚠️ **PASS_WITH_WAIVER**：存在 Major 问题但已在 §7 登记豁免（理由 + Owner + 后续整改时间）；本地校验门禁放行但需后续闭环
☐ ❌ **FAIL**：构建/测试不通过，或 §8 反作弊任一不通过，或存在未豁免的功能遗漏；本地校验门禁不放行
...
- {{...}}
- 进入独立的发布/SRE 流程处理：{{线上 dry-run / 灰度 / 报警 / 降级 / 回滚 等需要真实环境验证的项}}
