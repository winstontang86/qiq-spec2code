...
5. **复用胜过新建**：先扫描仓库，复用已有目录、依赖、风格、工具，**不要平地起高楼**。
...
把扫描结果摘要告诉用户，**让用户确认或纠正**后才能进入 Phase 1。
...
2. **测试执行**：`go test ./...` 必须通过；覆盖率不强制门槛但需如实记录。
3. **接口衔接抽样**：从入口到落库选 2~3 条核心链路，逐层 grep 验证传参与错误码贯通。
4. **配置完整性**：所有引用的配置 key 必须在 `configs/*` 中定义。
5. **trace_id / context 全链路**：抽样核对。
6. **遗漏回扫**：拉出原始技术方案功能列表与非功能需求列表，逐条核对是否落在某个 Task 中。
7. **结论**：`PASS` / `PASS_WITH_WAIVER` / `FAIL`。
...
- ✅ 所有外部 I/O 必须传 `context.Context` 并设置超时。
- ✅ 所有错误必须保留错误链（`%w`），不允许吞错。
